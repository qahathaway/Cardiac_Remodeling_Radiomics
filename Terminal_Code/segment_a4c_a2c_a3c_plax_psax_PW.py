...
def segmentChamber(videofile, dicomdir, view):
    ...
    if view == "a4c":
        ...
        plax_lv_post_wall_segs, preds = extract_segs(images, orig_images, model, sess, 5)
        np.save(outpath + '/' + videofile + '_lv_post_wall_Y5', np.array(plax_lv_post_wall_segs).astype('uint8'))
    j = 0
    ...
    plt.savefig(outpath + '/' + videofile + '_' + str(j) + '_' + 'CHOICE_FULL_segmentation_PW.png')
    plt.close() 
    plt.figure(figsize = (25, 25))
    plt.axis('off')
    plt.imshow(orig_images[0])
    plt.savefig(outpath + '/' + videofile + '_' + str(j) + '_' + 'CHOICE_FULL_original_PW.png')
    plt.close()   
    background = Image.open(outpath + '/' + videofile + '_' + str(j) + '_' + 'CHOICE_FULL_original_PW.png')
    overlay = Image.open(outpath + '/' + videofile + '_' + str(j) + '_' + 'CHOICE_FULL_segmentation_PW.png')
    background = background.convert("RGBA")
    overlay = overlay.convert("RGBA")
    outImage = Image.blend(background, overlay, 0.5)
    outImage.save(outpath + '/' + videofile + '_' + str(j) + '_' + 'CHOICE_FULL_overlay_PW.png', "PNG") 
    return 1
