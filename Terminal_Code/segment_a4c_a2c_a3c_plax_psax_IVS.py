...
def segmentChamber(videofile, dicomdir, view):
    ...
    if view == "a4c":
        ...
        plax_lv_inter_ventricular_segs, preds = extract_segs(images, orig_images, model, sess, 2)
        np.save(outpath + '/' + videofile + '_lv_inter_ventricular_G2', np.array(plax_lv_inter_ventricular_segs).astype('uint8'))
    j = 0
    ...
    plt.savefig(outpath + '/' + videofile + '_' + str(j) + '_' + 'CHOICE_FULL_segmentation_IVS.png')
    plt.close() 
    plt.figure(figsize = (25, 25))
    plt.axis('off')
    plt.imshow(orig_images[0])
    plt.savefig(outpath + '/' + videofile + '_' + str(j) + '_' + 'CHOICE_FULL_original_IVS.png')
    plt.close()   
    background = Image.open(outpath + '/' + videofile + '_' + str(j) + '_' + 'CHOICE_FULL_original_IVS.png')
    overlay = Image.open(outpath + '/' + videofile + '_' + str(j) + '_' + 'CHOICE_FULL_segmentation_IVS.png')
    background = background.convert("RGBA")
    overlay = overlay.convert("RGBA")
    outImage = Image.blend(background, overlay, 0.5)
    outImage.save(outpath + '/' + videofile + '_' + str(j) + '_' + 'CHOICE_FULL_overlay_IVS.png', "PNG") 
    return 1
...
def extract_segs(images, orig_images, model, sess, Seg1_label):
    segs = []
    preds = np.argmax(model.predict(sess, images[0:1])[0,:,:,:], 2)
    label_all = range(1, 8)
    label_good = [Seg1_label]
    for i in label_all:
    ...
    Seg1_segs = []
    for seg in segs:
        Seg1_seg = create_seg(seg, Seg1_label)      
        Seg1_segs.append(Seg1_seg)
    return Seg1_segs, preds

def main():
    viewfile = "Probabilities_CHOICE_FULL.txt"
    dicomdir = "CHOICE_FULL"
    viewlist_a2c = []
    viewlist_a3c = []
    viewlist_a4c = []
    viewlist_plax = []
