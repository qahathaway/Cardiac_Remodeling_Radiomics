...
MESA <- data.frame(read.csv(
  file = '/Users/quincy/Documents/Research/HVI/Radiomics - Automated/FINAL/Submission/Nature Cardiovascular Research/C-Statistic/COXPH_FullEcho_Multi_2.csv'))
...
fit <- surv_fit(Surv(MACE_months, MACE) ~ Age_x, data = final)
...
surv_object <- Surv(time = final$MACE_months, event = final$MACE)
fit.coxph <- coxph(surv_object ~Age_x, 
                   data = final)
